...
static void update_check_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    json_t* json = NULL;
    if(R_SUCCEEDED(res = http_download_json("https://api.github.com/repos/nick9088/remanage/releases/latest", &json, 16 * 1024))) {
                    ...
                    for(u32 i = 0; i < json_array_size(assets); i++) {
                        ...
                        if(json_is_object(val)) {
                            ...
                            if(json_is_string(assetName) && json_is_string(assetUrl)) {
                                if(strncmp(json_string_value(assetName), fs_get_3dsx_path() != NULL ? "ReManage.3dsx" : "ReManage.cia", json_string_length(assetName)) == 0) {
    ...
    if(hasUpdate) {
        action_install_url("Update ReManage to the latest version?", updateURL, fs_get_3dsx_path(), NULL, NULL, NULL, NULL);
    } else {
        if(R_FAILED(res)) {
